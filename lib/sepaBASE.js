...
class sepaBASE {
   ...
   /**
    * static function: checks if id string is valid and length max. 35 chars
    * @param {string} id - id to validate
    * @return {boolean} - true if valid, false if invalid
    */
    static validateE2EId(id) {
        // may not be empty
        if (id === '') {
            return false;
        }
        ...
        // pattern matching
        if ( ! id.match(/^[abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ 0123456789/?:().,'+-]+$/) ) {
            return false;
        }
        return true;
    }
   ...
   /**
    * Get md5 hash of last gernerated xml string
    * @returns {string} - md5 hash value in lower case
    */
    get getMsgHash() {
        // return String(this._payload_md5.toLowerCase());
        const crypto = require('crypto');
        return crypto.createHash('md5').update(this.getMsgAsXmlString()).digest('hex').toLowerCase();
    }
